...
public class MailSender {
    ...
    public int sendMail(String userMail, MailCommand command , Map<String,String> msgProps){
        ...
        try {
            ...
            if(command == MailCommand.SUBSCRIBE){
                message.setSubject("Ugo's dev blog 새로운 컨텐츠가 추가되었습니다.");
            }else if(command == MailCommand.SIGNUP){
                message.setSubject(mailProperties.getSubject());
            }else{
                message.setSubject("Ugo's dev blog 아이디/비밀번호 입니다.");
            }
